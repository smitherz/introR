...
## 
## data:  x and y
## t = -6.8916, df = 36.921, p-value = 3.999e-08
...
##Excercise 1
For this exercise, let's start to look at some of the statistical tests and relationships in some USGS data.

Start by using 'dataRetrieval' to gather data from NWIS:
...
{% highlight text %}
## Error in eval(expr, envir, enclos): could not find function "readNWISDaily"
{% endhighlight %}
...
{% highlight text %}
## Error in eval(expr, envir, enclos): could not find function "readNWISInfo"
{% endhighlight %}
...
{% highlight text %}
## Error in INFO$shortName <- "Choptank River": object 'INFO' not found
{% endhighlight %}
...
{% highlight text %}
## Error in eval(expr, envir, enclos): could not find function "readNWISSample"
{% endhighlight %}

1. First, lets take a look at the relationship between discharge and inorganic N.  Add a section to your script that tests for a difference in the mean value of TURB, NTL, PTL, CHLA, and SECMEAN between man made and natural lakes.  For this we will have the script save each test to a new object and print the results to the screen.
